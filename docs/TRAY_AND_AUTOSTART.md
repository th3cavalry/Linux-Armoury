...
2. Your custom settings are applied immediately
3. Access quick controls via tray icon when needed
4. Close the window when done → Stays running in background

### Scenario 2: Gaming Session
1. Right-click tray icon → Select "Performance" profile
2. Optionally open window to monitor temps
3. Your settings persist even after closing the window
4. Switch back to "Balanced" from tray when done

### Scenario 3: Battery Conservation
1. Right-click tray icon → Select "Battery Saver" profile
2. Application automatically adjusts TDP and refresh rate
3. Settings remain active as long as app is running
4. No need to keep window open
...
2. Select "Quit"
3. OR run: `killall python3` (force kill)

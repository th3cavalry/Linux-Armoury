...
class OverclockingController:
    ...
    def _check_ryzenadj(self) -> bool:
        ...
        try:
            result = subprocess.run(
                ["which", "ryzenadj"], capture_output=True, text=True
            )
    ...
    def _check_cpupower(self) -> bool:
        ...
        try:
            result = subprocess.run(
                ["which", "cpupower"], capture_output=True, text=True
            )
            return result.returncode == 0
        except Exception:
            return False
    ...
    def _write_sysfs(self, path: str, value: str) -> bool:
        ...
        try:
            ...
            try:
                result = subprocess.run(
                    ["pkexec", "tee", path], input=value, capture_output=True, text=True
                )
                return result.returncode == 0
            except Exception:
                return False
    ...
    def get_available_governors(self) -> List[str]:
        ...
        governors = self._read_sysfs(
            f"{self.SYSFS_CPU_BASE}/cpu0/cpufreq/scaling_available_governors"
        )
        if governors:
    ...
    def set_cpu_governor(self, governor: str) -> bool:
        ...
        if self.cpupower_available:
            success, _, _ = self._run_privileged(
    ...
    def set_cpu_frequency_limits(
        self, min_mhz: Optional[int] = None, max_mhz: Optional[int] = None
    ) -> bool:
    ...
    def get_ryzenadj_info(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            result = subprocess.run(
                ["pkexec", "ryzenadj", "-i"], capture_output=True, text=True, timeout=10
            )

...
try:
    from .fan_curve_editor import FanCurveWidget, FanCurveEditorDialog, get_preset_curve
    HAS_FAN_CURVE_EDITOR = True
except ImportError:
    HAS_FAN_CURVE_EDITOR = False

try:
    from .modules.session_stats import get_session_stats, SessionStatistics
...
try:
    from .modules.overclocking_control import OverclockingController, TDP_PRESETS, CPUInfo, GPUInfo
    HAS_OVERCLOCKING = True
except ImportError:
    HAS_OVERCLOCKING = False

try:
    from .modules.gpu_control import GpuController, GpuMode, GpuPowerStatus, GpuLiveStats, GpuSwitchingStatus, get_controller as get_gpu_controller
    HAS_GPU_CONTROL = True
except ImportError:
    HAS_GPU_CONTROL = False

try:
    from .modules.system_monitor import SystemMonitor, get_monitor, CpuStats, MemoryStats, DiskStats, NetworkStats, ProcessInfo, SystemOverview
...
class MainWindow(Adw.ApplicationWindow):
    ...
    def on_close_request(self, window):
        ...
        if hasattr(app, 'show_tray_notification'):
            app.show_tray_notification("Linux Armoury", "Running in background. Right-click tray icon to quit.")
    ...
    def setup_ui(self):
        ...
        sidebar_button.connect("clicked", lambda b: self.split_view.set_show_sidebar(
    ...
    def create_dashboard_page(self):
        ...
        if HAS_SESSION_STATS:
            ...
            duration_icon = Gtk.Image.new_from_icon_name("preferences-system-time-symbolic")
    ...
    def create_monitor_page(self):
        ...
        freq_range_label = Gtk.Label(label=f"{cpu.min_freq_mhz:.0f}-{cpu.max_freq_mhz:.0f} MHz")
        ...
        self.mon_load_row.set_subtitle(f"{cpu.load_1min:.2f}, {cpu.load_5min:.2f}, {cpu.load_15min:.2f}")
        ...
        for disk in disks:
            ...
            row.set_subtitle(f"{disk.used_gb:.1f} / {disk.total_gb:.1f} GB ({disk.usage_percent:.1f}%)")
        ...
        for iface in interfaces:
            ...
            dl_rate_label = Gtk.Label(label=self.sys_monitor.format_bytes_rate(iface.recv_rate))
            ...
            ul_rate_label = Gtk.Label(label=self.sys_monitor.format_bytes_rate(iface.send_rate))
        ...
        for proc in processes:
            ...
            proc_icon = Gtk.Image.new_from_icon_name("application-x-executable-symbolic")
    ...
    def create_performance_page(self):
        ...
        profiles_group.set_description("Select a performance profile to balance power and battery")
        
        # Power profiles with icons and TDP info
        profiles = [
            ("emergency", "Emergency", "10W @ 30Hz", "Critical battery preservation", "battery-caution-symbolic"),
            ("battery", "Battery Saver", "18W @ 30Hz", "Maximum battery life", "battery-full-symbolic"),
            ("efficient", "Efficient", "30W @ 60Hz", "Balanced efficiency", "battery-good-symbolic"),
            ("balanced", "Balanced", "40W @ 90Hz", "Default balanced mode", "power-profile-balanced-symbolic"),
            ("performance", "Performance", "55W @ 120Hz", "High performance", "power-profile-performance-symbolic"),
            ("gaming", "Gaming", "70W @ 180Hz", "Gaming optimized", "applications-games-symbolic"),
            ("maximum", "Maximum", "90W @ 180Hz", "Absolute maximum power", "emblem-important-symbolic"),
        ]
        ...
        if detected_profile:
            ...
            current_profile = profile_map.get(detected_profile, self.settings.get("current_power_profile", "balanced"))
        else:
    ...
    def create_gpu_page(self):
        ...
        if self.gpu_controller and self.gpu_controller.supergfxctl_available:
            ...
            switch_group.set_description("Switch between integrated and discrete graphics")
            ...
            vendor_icon = Gtk.Image.new_from_icon_name("applications-multimedia-symbolic")
            ...
            if status.supported_modes:
                ...
                select_icon = Gtk.Image.new_from_icon_name("preferences-system-symbolic")
                ...
                if status.current_mode and status.current_mode in status.supported_modes:
                    mode_select_row.set_selected(status.supported_modes.index(status.current_mode))
        ...
        stats_group.set_description("Real-time GPU monitoring (updates every 2 seconds)")
    ...
    def create_display_page(self):
        ...
        refresh_group.set_description("Higher refresh rates provide smoother visuals but use more power")
        
        # Refresh rate profiles with detailed info
        profiles = [
            ("30", "30 Hz", "Battery saving • Best for static content", "battery-full-symbolic"),
            ("60", "60 Hz", "Standard • Good balance", "display-symbolic"),
            ("90", "90 Hz", "Smooth • Everyday use", "preferences-desktop-display-symbolic"),
            ("120", "120 Hz", "High refresh • Gaming and productivity", "video-display-symbolic"),
            ("180", "180 Hz", "Maximum • Competitive gaming", "applications-games-symbolic"),
        ]
    ...
    def create_cooling_page(self):
            ...
            try:
                ...
                if fan.is_supported():
                    ...
                    if fan.has_fan_curves():
                        ...
                        curve_icon = Gtk.Image.new_from_icon_name("utilities-tweak-tool-symbolic")
                        curve_row.add_prefix(curve_icon)
                        
                        self.fan_curve_switch = Gtk.Switch()
                        self.fan_curve_switch.set_valign(Gtk.Align.CENTER)
                        self.fan_curve_switch.connect("state-set", self.on_fan_curve_toggled)
                        ...
                        if HAS_FAN_CURVE_EDITOR:
                            ...
                            edit_row.set_subtitle("Visually customize your fan response curve")
        ...
        if HAS_FAN_CURVE_EDITOR and not HAS_FAN_CONTROL:
            ...
            curve_preview_group.set_description("Visual fan curve editor (requires asusd for application)")
    ...
    def create_overclocking_page(self):
        ...
        if not HAS_OVERCLOCKING:
            ...
            info_row.set_subtitle("Install overclocking_control.py to enable this feature")
        ...
        self.oc_cpu_freq_row.set_subtitle(f"{cpu_info.current_freq_mhz:.0f} MHz (max: {cpu_info.max_freq_mhz:.0f} MHz)")
        freq_icon = Gtk.Image.new_from_icon_name("speedometer-symbolic")
        ...
        turbo_row.set_subtitle("Allow CPU to exceed base frequency when thermal headroom permits")
        ...
        if self.oc_controller.ryzenadj_available:
            ...
            for preset_name, preset_values in TDP_PRESETS.items():
                ...
                btn_icon = Gtk.Image.new_from_icon_name(preset_colors.get(preset_name, "power-profile-balanced-symbolic"))
                btn_label = Gtk.Label(label=preset_name.title())
                btn_box.append(btn_icon)
                btn_box.append(btn_label)
                btn.set_child(btn_box)
                btn.add_css_class("pill")
                btn.set_tooltip_text(f"STAPM: {preset_values['stapm']}W, Fast: {preset_values['fast']}W, Slow: {preset_values['slow']}W")
            ...
            self.stapm_scale = Gtk.Scale.new_with_range(Gtk.Orientation.HORIZONTAL, 5, 65, 1)
            ...
            fast_icon = Gtk.Image.new_from_icon_name("power-profile-performance-symbolic")
            fast_row.add_prefix(fast_icon)
            
            self.fast_scale = Gtk.Scale.new_with_range(Gtk.Orientation.HORIZONTAL, 5, 80, 1)
            ...
            slow_icon = Gtk.Image.new_from_icon_name("power-profile-power-saver-symbolic")
            slow_row.add_prefix(slow_icon)
            
            self.slow_scale = Gtk.Scale.new_with_range(Gtk.Orientation.HORIZONTAL, 5, 65, 1)
            ...
            self.temp_limit_scale = Gtk.Scale.new_with_range(Gtk.Orientation.HORIZONTAL, 70, 105, 1)
        ...
        if self.oc_controller.amd_gpu_path:
            ...
            if gpu_info:
                ...
                self.oc_gpu_status_row.set_subtitle(f"Clock: {gpu_info.current_gpu_clock_mhz} MHz | Temp: {gpu_info.gpu_temp_c}°C")
                ...
                perf_levels = ["auto", "low", "high", "manual", "profile_standard", "profile_peak"]
                ...
                if profiles:
                    ...
                    profile_row.set_subtitle("Optimized settings for different workloads")
                    profile_icon = Gtk.Image.new_from_icon_name("applications-games-symbolic")
                    ...
                    profile_row.connect("notify::selected", self.on_gpu_power_profile_changed)
    ...
    def on_apply_tdp_clicked(self, button):
        ...
        success = True
        if not self.oc_controller.set_ryzenadj_tdp(stapm_limit=stapm, fast_limit=fast, slow_limit=slow):
            success = False
        if not self.oc_controller.set_ryzenadj_temp_limit(temp):
            success = False
        
        if success:
            self.show_toast(f"Applied TDP: STAPM={stapm}W, Fast={fast}W, Slow={slow}W, Temp={temp}°C")
        else:
        ...
        if not HAS_KEYBOARD_CONTROL:
            ...
            try:
                ...
                if not kbd.is_supported():
                    ...
                    unsupported_row.set_subtitle("No compatible keyboard backlight found")
                    warn_icon = Gtk.Image.new_from_icon_name("dialog-warning-symbolic")
                    unsupported_row.add_prefix(warn_icon)
                    status_group.add(unsupported_row)
                    page.add(status_group)
                    ...
                    brightness_row.set_subtitle(f"Current: {current_brightness} / {max_brightness}")
                    bright_icon = Gtk.Image.new_from_icon_name("display-brightness-symbolic")
                    brightness_row.add_prefix(bright_icon)
                    
                    # Brightness buttons
                    brightness_box = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=4)
                    ...
                    if kbd.has_rgb():
                        ...
                        color_group.set_description("Customize keyboard backlight color")
                        ...
                        for color_name, hex_color, tooltip in colors:
                            ...
                            btn.connect("clicked", self.on_kbd_color_clicked, color_name)
                        ...
                        picker_icon = Gtk.Image.new_from_icon_name("color-select-symbolic")
        ...
        if not HAS_BATTERY_CONTROL:
            ...
            try:
                ...
                if not battery.is_supported():
                    ...
                    unsupported_row.set_subtitle("Your device may not support charge limit control")
                    warn_icon = Gtk.Image.new_from_icon_name("dialog-warning-symbolic")
                    unsupported_row.add_prefix(warn_icon)
                    status_group.add(unsupported_row)
                    page.add(status_group)
                    ...
                    charge_icon = Gtk.Image.new_from_icon_name(
                        "battery-full-charging-symbolic" if status == "Charging" else "battery-symbolic")
                    ...
                    limit_icon = Gtk.Image.new_from_icon_name("battery-caution-symbolic")
    ...
    def create_automation_page(self):
        ...
        profile_group.set_description("Automatically switch profiles based on power state")
        
        # Enable/disable auto-switching
        enable_row = Adw.SwitchRow()
        enable_row.set_title("Enable Auto-Switching")
        enable_row.set_subtitle("Automatically change profile when plugging/unplugging AC")
        ...
        refresh_group.set_description("Automatically adjust refresh rate based on power state")
        ...
        refresh_enable_row.set_active(self.load_config().get('auto_refresh_enabled', False))
        refresh_enable_row.connect("notify::active", self.on_auto_refresh_enabled_changed)
        ...
        refresh_rates = Gtk.StringList.new(["30 Hz", "60 Hz", "90 Hz", "120 Hz", "180 Hz"])
        ...
        bat_refresh_row.connect("notify::selected", self.on_auto_battery_refresh_changed)
        ...
        temp_group.set_description("Automatically reduce power when temperature is high")
        
        # Enable temperature throttling
        temp_enable_row = Adw.SwitchRow()
        temp_enable_row.set_title("Temperature Throttling")
        temp_enable_row.set_subtitle("Reduce TDP when CPU temperature exceeds threshold")
        temp_enable_row.set_active(self.load_config().get('temp_throttle_enabled', True))
        ...
        temp_threshold_row.set_subtitle("Throttle when CPU exceeds this temperature (°C)")
    ...
    def create_system_info_page(self):
        ...
        if model_info:
            if model_info.get('vendor'):
                ...
                vendor_row.add_prefix(Gtk.Image.new_from_icon_name("emblem-system-symbolic"))
            ...
            if model_info.get('product'):
                ...
                product_row.add_prefix(Gtk.Image.new_from_icon_name("computer-laptop-symbolic"))
            ...
            if model_info.get('board'):
                ...
                board_row.add_prefix(Gtk.Image.new_from_icon_name("application-x-firmware-symbolic"))
        ...
        kernel_row.add_prefix(Gtk.Image.new_from_icon_name("utilities-terminal-symbolic"))
        ...
        desktop_row.add_prefix(Gtk.Image.new_from_icon_name("preferences-desktop-symbolic"))
        ...
        for i, gpu in enumerate(gpus):
            ...
            gpu_row.set_subtitle(f"{gpu.get('name', 'Unknown')} ({gpu.get('type', 'Unknown')})")
        ...
        usage_row.add_prefix(Gtk.Image.new_from_icon_name("utilities-system-monitor-symbolic"))
        ...
        for device in storage_devices[:3]:  # Limit to first 3 devices
            ...
            storage_row.add_prefix(Gtk.Image.new_from_icon_name("drive-harddisk-symbolic"))
    ...
    def on_open_fan_curve_editor(self, button):
        ...
        dialog = FanCurveEditorDialog(
            fan_name="CPU Fan",
            on_apply=self.on_fan_curve_applied
        )
        dialog.present(self)
    ...
    def on_fan_curve_applied(self, fan_name: str, curve_data: list):
        ...
        if HAS_FAN_CONTROL:
            ...
            if hasattr(fan, 'set_fan_curve'):
                ...
                self.show_toast(f"Fan curve saved for {fan_name} (requires asusd to apply)")
        else:
    ...
    def show_session_details(self, button):
        ...
        session_label.set_markup(f"<b>Session started:</b> {summary.get('session_start', 'N/A')}")
        session_label.set_halign(Gtk.Align.START)
        content_box.append(session_label)
        
        duration_label = Gtk.Label()
        duration_label.set_markup(f"<b>Duration:</b> {summary.get('session_duration', 'N/A')}")
    ...
    def refresh_status(self):
        ...
        if hasattr(self, 'dash_battery_label') and battery is not None:
            ...
            if on_ac:
                self.dash_battery_icon.set_from_icon_name("battery-full-charging-symbolic")
            elif battery < 20:
        ...
        if hasattr(self, 'perf_tdp_row'):
            if tdp:
                self.perf_tdp_row.set_subtitle(f"{tdp} W • Profile: {current_profile.capitalize()}")
            else:
                self.perf_tdp_row.set_subtitle(f"Profile: {current_profile.capitalize()}")
            ...
            try:
                ...
                if hasattr(self, 'gpu_clock_row'):
                    if stats.gpu_clock_max_mhz > 0:
                        self.gpu_clock_row.set_subtitle(f"{stats.gpu_clock_mhz} / {stats.gpu_clock_max_mhz} MHz")
                    else:
                ...
                if hasattr(self, 'gpu_mem_clock_row'):
                    if stats.mem_clock_max_mhz > 0:
                        self.gpu_mem_clock_row.set_subtitle(f"{stats.mem_clock_mhz} / {stats.mem_clock_max_mhz} MHz")
                    else:
                        self.gpu_mem_clock_row.set_subtitle(f"{stats.mem_clock_mhz} MHz")
                ...
                if hasattr(self, 'gpu_vram_row'):
                    if stats.vram_total_mb > 0:
                        self.gpu_vram_row.set_subtitle(f"{stats.vram_used_mb} / {stats.vram_total_mb} MB ({stats.mem_usage_percent}%)")
                ...
                if hasattr(self, 'gpu_power_draw_row'):
                    if stats.power_limit_w > 0:
                        self.gpu_power_draw_row.set_subtitle(f"{stats.power_draw_w:.1f} / {stats.power_limit_w:.0f} W")
                    else:
                        self.gpu_power_draw_row.set_subtitle(f"{stats.power_draw_w:.1f} W")
                
                # Update fan speed
                if hasattr(self, 'gpu_fan_row'):
                    if stats.fan_speed_rpm > 0:
                        self.gpu_fan_row.set_subtitle(f"{stats.fan_speed_rpm} RPM ({stats.fan_speed_percent}%)")
                    else:
                        self.gpu_fan_row.set_subtitle("N/A or Passive")
                
                # Update GPU switching status
                if hasattr(self, 'gpu_power_row') and self.gpu_controller.supergfxctl_available:
                    switch_status = self.gpu_controller.get_switching_status()
                    self.gpu_power_row.set_subtitle(switch_status.power_status.value.capitalize())
                    # Update badge
                    if hasattr(self, 'gpu_power_badge'):
                        self.gpu_power_badge.set_label(switch_status.power_status.value.upper())
        ...
        if HAS_SYSTEM_MONITOR and hasattr(self, 'sys_monitor') and self.sys_monitor:
            try:
                ...
                if hasattr(self, 'mon_cpu_usage_row'):
                    self.mon_cpu_usage_row.set_subtitle(f"{cpu_stats.usage_percent:.1f}%")
                    self.mon_cpu_usage_bar.set_value(cpu_stats.usage_percent)
                
                if hasattr(self, 'mon_cpu_freq_row'):
                    self.mon_cpu_freq_row.set_subtitle(f"{cpu_stats.current_freq_mhz:.0f} MHz")
                
                if hasattr(self, 'mon_load_row'):
                    self.mon_load_row.set_subtitle(f"{cpu_stats.load_1min:.2f}, {cpu_stats.load_5min:.2f}, {cpu_stats.load_15min:.2f}")
                ...
                if hasattr(self, 'mon_ram_row'):
                    self.mon_ram_row.set_subtitle(f"{mem_stats.used_mb:,} / {mem_stats.total_mb:,} MB ({mem_stats.usage_percent:.1f}%)")
                    self.mon_ram_bar.set_value(mem_stats.usage_percent)
                
                if hasattr(self, 'mon_ram_available_row'):
                    self.mon_ram_available_row.set_subtitle(f"{mem_stats.available_mb:,} MB")
                
                if hasattr(self, 'mon_ram_cached_row'):
                    self.mon_ram_cached_row.set_subtitle(f"{mem_stats.cached_mb:,} MB / {mem_stats.buffers_mb:,} MB")
                
                if hasattr(self, 'mon_swap_row'):
                    if mem_stats.swap_total_mb > 0:
                        self.mon_swap_row.set_subtitle(f"{mem_stats.swap_used_mb:,} / {mem_stats.swap_total_mb:,} MB ({mem_stats.swap_usage_percent:.1f}%)")
                ...
                if hasattr(self, 'mon_net_rows'):
                    ...
                    for iface in net_stats:
                        if iface.interface in self.mon_net_rows:
                            ...
                            rows['dl_row'].set_subtitle(self.sys_monitor.format_bytes(iface.bytes_recv))
                            rows['ul_row'].set_subtitle(self.sys_monitor.format_bytes(iface.bytes_sent))
                            rows['dl_rate'].set_text(self.sys_monitor.format_bytes_rate(iface.recv_rate))
                            rows['ul_rate'].set_text(self.sys_monitor.format_bytes_rate(iface.send_rate))
                            
                            # Update status
                            status = "Connected" if iface.is_up else "Disconnected"
                            rows['expander'].set_subtitle(f"{status} • {iface.ip_address or 'No IP'}")
                            rows['status_badge'].set_text("UP" if iface.is_up else "DOWN")
        ...
        if config.get('temp_throttle_enabled', True) and cpu_temp:
            ...
            if cpu_temp >= threshold:
                ...
                if current in ['performance', 'gaming', 'maximum']:
                    ...
                    self.show_toast(f"⚠️ Temp throttling: {cpu_temp:.0f}°C → Balanced profile")
    ...
    def show_about(self, action, param):
        ...
        # Add credits
        about.add_credit_section("Powered By", [
            "asusctl/asusd",
            "supergfxctl",
            "GTK4/libadwaita",
        ])
...
def main():
    ...
    parser.add_argument('--version', '-v', action='version',
                       version=f'Linux Armoury {Config.VERSION}')
